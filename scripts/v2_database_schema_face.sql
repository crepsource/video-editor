...
CREATE TABLE persons (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  video_collection_id UUID, -- groups persons across related videos
  
  -- Person identification
  person_label TEXT, -- "main_creator", "travel_companion", "local_guide", etc.
  face_encoding BYTEA, -- facial recognition encoding for consistency
  appearance_description TEXT, -- "person in red jacket", "blonde woman with sunglasses"
  
  -- Appearance tracking
  clothing_descriptions TEXT[], -- ["red_jacket_white_shirt", "blue_dress", "swimwear"]
  distinguishing_features TEXT[], -- ["sunglasses", "backpack", "hat"]
  
  -- Role analysis
  person_role TEXT, -- main_subject, companion, local, passerby
  importance_level INTEGER, -- 1=main focus, 2=secondary, 3=background
  
  -- Statistics
  total_appearances INTEGER DEFAULT 0,
  first_appearance_timestamp DECIMAL,
  last_appearance_timestamp DECIMAL,
  
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW()
);

-- Person appearances in specific frames
CREATE TABLE person_appearances (
  ...
  -- Detection details
  confidence_score DECIMAL, -- 0-1 confidence this is the same person
  bounding_box JSONB, -- {"x": 120, "y": 200, "width": 300, "height": 400}
  face_visible BOOLEAN DEFAULT true,
  body_visible BOOLEAN DEFAULT true,
  
  -- Position and prominence
  position_in_frame TEXT, -- center, left, right, background, foreground
  prominence_score DECIMAL, -- 0-10 how prominent in the frame
  is_main_subject BOOLEAN DEFAULT false,
  
  -- Action and context
  apparent_action TEXT, -- walking, sitting, eating, pointing, smiling
  interaction_context TEXT, -- solo, with_others, addressing_camera
  emotional_expression TEXT, -- happy, excited, contemplative, surprised
  
  -- Appearance details for this frame
  clothing_description TEXT,
  accessories TEXT[], -- ["sunglasses", "hat", "backpack"]
  pose_description TEXT,
  
  created_at TIMESTAMP DEFAULT NOW()
);

-- Person consistency tracking across scenes
CREATE TABLE person_continuity (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  person_id UUID REFERENCES persons(id),
  scene_id UUID REFERENCES scenes(id),
  
  -- Continuity metrics
  clothing_consistency_score DECIMAL, -- 0-1 same outfit
  appearance_consistency_score DECIMAL, -- 0-1 same general look
  role_consistency_score DECIMAL, -- 0-1 same narrative role
  
  -- Scene-specific analysis
  scene_importance DECIMAL, -- 0-10 how important person is in this scene
  narrative_function TEXT, -- protagonist, guide, companion, background
  screen_time_percentage DECIMAL, -- % of scene person is visible
...
CREATE TABLE timeline_clips (
  ...
  -- Person context
  featured_persons UUID[], -- array of person IDs visible in clip
  main_person_id UUID REFERENCES persons(id), -- primary person in clip
  person_interaction_type TEXT, -- solo, group, addressing_camera
...
-- Person tracking indexes
CREATE INDEX idx_persons_collection ON persons(video_collection_id);
CREATE INDEX idx_person_appearances_person ON person_appearances(person_id);
CREATE INDEX idx_person_appearances_frame ON person_appearances(frame_id);
CREATE INDEX idx_person_appearances_confidence ON person_appearances(confidence_score DESC);
CREATE INDEX idx_person_continuity_person ON person_continuity(person_id);
CREATE INDEX idx_timeline_clips_main_person ON timeline_clips(main_person_id);
...
SELECT 
  ...
  s.story_importance,
  pa.person_id as featured_person,
  pa.is_main_subject as person_is_main_subject
FROM frames_enhanced f
LEFT JOIN frame_scenes fs ON f.id = fs.frame_id
LEFT JOIN scenes s ON fs.scene_id = s.id
LEFT JOIN person_appearances pa ON f.id = pa.frame_id
WHERE 
...
SELECT 
  ...
  COUNT(DISTINCT tc.main_person_id) as unique_persons_featured,
  t.created_at
...
-- Person-focused view for creator analytics
CREATE VIEW person_story_arc AS
SELECT 
  p.id as person_id,
  p.person_label,
  p.person_role,
  COUNT(pa.id) as total_appearances,
  AVG(pa.prominence_score) as avg_prominence,
  MIN(f.timestamp) as first_appearance,
  MAX(f.timestamp) as last_appearance,
  ARRAY_AGG(DISTINCT pa.apparent_action) as actions_performed,
  ARRAY_AGG(DISTINCT pa.emotional_expression) as expressions_shown
FROM persons p
JOIN person_appearances pa ON p.id = pa.person_id
JOIN frames_enhanced f ON pa.frame_id = f.id
GROUP BY p.id, p.person_label, p.person_role;